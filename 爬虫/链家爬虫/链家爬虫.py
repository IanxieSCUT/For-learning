...
# 获取某市区域的所有链接
def get_areas(url,url_zufang):
    print('start grabing areas')
    headers = {
        'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.108 Safari/537.36'}
    resposne = requests.get(url_zufang, headers=headers)
...
def get_house_info(area, url):
    ...
    try:
        ...
        for i in range(30):
            ...
            with open('链家深圳租房.txt','a',encoding='utf-8') as f: #这里记得改文件名
...
def main():
    ...
    url_zufang = url + 'zufang/'
    get_areas(url,url_zufang)
